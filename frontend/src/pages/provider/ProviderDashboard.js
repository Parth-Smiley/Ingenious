...
function ProviderDashboard({ onLogout }) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const transformedServices = Array.isArray(data) ? data.map(service => ({
          ...
          status: service.status ? service.status.toLowerCase() : "pending",
          date: new Date(service.created_at || service.updated_at || Date.now()).toISOString().split('T')[0],
          base_url: service.base_url,
          endpoint_path: service.endpoint_path
        })) : [];
        
        setRegisteredServices(transformedServices);
      } else {
        // Fallback to sample data if API fails
        const sampleServices = [
          { id: 1, name: "City Complaint Service", domain: "city", status: "approved", date: "2024-01-15", base_url: "https://api.city.gov", endpoint_path: "/complaints" },
          { id: 2, name: "Health Appointment System", domain: "health", status: "pending", date: "2024-01-10", base_url: "https://health.service", endpoint_path: "/appointments" },
          { id: 3, name: "Agriculture Advisory", domain: "agriculture", status: "rejected", date: "2024-01-05", base_url: "https://agri.gov", endpoint_path: "/advisory" },
        ];
        setRegisteredServices(sampleServices);
      }
    } catch (error) {
  ...
  // Register service - Your backend logic
  const registerService = async () => {
    ...
    try {
      ...
      const newService = {
        id: registeredServices.length > 0 ? Math.max(...registeredServices.map(s => s.id)) + 1 : 1,
        name: name,
        domain: domain,
        status: "pending",
  ...
  const stats = {
    ...
    approvedServices: registeredServices.filter(s => s.status === 'approved').length,
    pendingServices: registeredServices.filter(s => s.status === 'pending').length,
    rejectedServices: registeredServices.filter(s => s.status === 'rejected').length,
  };
      ...
      <aside className="provider-sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          >
            ...
            <span className="notification-count">{stats.totalServices}</span>
          </button>
